...
namespace LibEmiddle.Abstractions
{
    ...
    {
        DoubleRatchetSession InitializeSessionAsSenderAsync(byte[] sharedKeyFromX3DH, byte[] recipientInitialPublicKey, string sessionId);
        DoubleRatchetSession InitializeSessionAsReceiverAsync(byte[] sharedKeyFromX3DH, KeyPair receiverInitialKeyPair, byte[] senderEphemeralKeyPublic, string sessionId);
